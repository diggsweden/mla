    ...
    if (showJoin && !joinLoading) {
      ...
      Object.values(entities)
        ...
        .forEach((e) => {
          ...
          if (matchRules != null) {
            const existingId = findId(
              e,
              matchRules,
              Object.values(entities)
                .flat()
                .filter((x) => x.InternalId !== e.InternalId)
            );
            if (existingId != null) {
          ...
          const updateLink = produce(l, (draft) => {
            ...
            if (matchRules != null) {
              const existingId = findId(
                ...
                Object.values(links)
                  .flat()
                  .filter((x) => x.InternalId !== l.InternalId)
              );
              if (existingId != null) {
    ...
    <>
      <RibbonMenuSection title={t("tools")}>
        ...
        <RibbonMenuButton
          label={t("merge")}
          title={t("merge desc")}
          disabled={Object.keys(entities).length + Object.keys(links).length === 0}
          onClick={() => {
            setShowJoin(true);
          }}
          iconName="join_left"
      ...
      {showJoin && (
        <Modal
          mode={joinResult.length > 0 ? "accept" : "ok"}
          show={showJoin}
          title={t("merge")}
          onNegative={() => {
            setShowJoin(false);
        ...
        >
          ...
          {!joinLoading && joinResult.length > 0 && (
            <div className="m-text-left m-px-3 m-py-2">
              <p>{t("merge")}</p>
