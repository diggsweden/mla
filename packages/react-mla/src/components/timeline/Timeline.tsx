...
function Timeline(props: Props) {
  ...
  const getNextDate = useCallback(
    (direction: number, current: ITimeSpan) => {
  ...
  const animateDate = useCallback(
    (target: DateTime, date: ITimeSpan, updates: number) => {
  ...
  return (
    <div className="m-flex-1 m-flex m-pointer-events-none">
      {historyMode && (
        <div className="m-w-full m-self-end m-mb-5">
          <div className={"m-w-full " + props.className}>
            <div className="m-select-none m-absolute m-bottom-16 m-h-11 m-left-1/4 m-right-1/4">
              ...
              <div className="m-absolute m-h-full m-w-full m-flex m-justify-center m-pointer-events-none">
                <div className="m-h-full m-w-0.5 m-bg-blue-800 m-z-10"></div>
              </div>
              <div
                className={isDay ? "m-hidden " : "m-absolute m-h-full m-top-0 m-opacity-30 m-left-1/2 m-z-5 m-bg-blue-300 m-border-x m-border-blue-800 m-z-10 m-pointer-events-none"}
            ...
            <div className="m-flex m-flex-row m-justify-center m-flex-nowrap m-shrink-0 m-grow-0 m-mt-4 m-cursor-pointer">
              <span
                className="m-mr-5 m-backdrop-blur-md m-bg-white/3"
                title={t("back")}
                onClick={() => {
                  if (play === 0) {
                    next(-1);
              ...
              <span
                className="m-backdrop-blur-md m-bg-white/3"
                title={t("pause play")}
                onClick={() => {
                  setPlay(0);
                }}
              ...
              <span
                className="m-backdrop-blur-md m-bg-white/3"
                title={t("play activities")}
                onClick={() => {
                  if (getNextDate(1, currentDate) != undefined) {
                    activatePlay(1);
                  }
                }}
              >
              ...
              <span
                className="m-ml-5 m-backdrop-blur-md m-bg-white/3"
                title={t("forward")}
                onClick={() => {
                  if (play === 0) {
                    next(1);
