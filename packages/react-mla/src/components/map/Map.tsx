  ...
  return (
    <div className={props.className + " m-bg-white"}>
      <div className="m-h-full m-w-full m-relative">
        {placeEntityId && (
          <div className="m-absolute m-h-full m-w-full m-z-30 m-opacity-75 m-pointer-events-none">
            <p className="m-bg-white m-mx-20 m-mt-5 m-text-center m-rounded-md">{t("click to place", { name: getEntity(placeEntityId)!.LabelShort })}</p>
          </div>
        )}
      ...
      {map && showMap && geoFeatures.map((g) => <GeoFeature key={g.Id} geo={g} map={map}></GeoFeature>)}
    </div>
  );
}
