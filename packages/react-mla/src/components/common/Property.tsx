...
function Property (props: Props) {
  ...
  function getInput () {
    ...
    switch (config.FieldType) {
      ...
      case 'Number': return <input id={key} ref={inputRef} readOnly={readOnly ?? false} type="number" required={config.Required} value={value ? Number(value) : 0} onChange={(e) => { handleChange(e.target.value ? Number(e.target.value) : undefined) }} className={inputClass}></input>
