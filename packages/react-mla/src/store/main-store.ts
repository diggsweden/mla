...
export interface MainState {
  ...
  setDrawingTool: (tool: DrawingTool | undefined) => void;
...
const useMainStore = create<MainState>((set, get) => ({
  ...
  save: () => {
    ...
    const file = {
      Entities: Object.values(get().entities)
        .flat()
        .map((x) => removeInternals(x)),
      Links: Object.values(get().links)
        .flat()
        .map((x) => removeInternals(x)),
  ...
  open: (json: string) => {
    ...
    if (parsed.Drawings) {
      ...
      // Parse the shape data from the drawings JSON string and update the shapes array
      try {
        const shapes = JSON.parse(parsed.Drawings);
